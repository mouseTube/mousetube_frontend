...
export const useSoftwareStore = defineStore('software', {
  ...
  actions: {
    ...
    async fetchSoftwareVersionById(id: number) {
      ...
      try {
        const apiBaseUrl = useApiBaseUrl();
        const res: AxiosResponse<SoftwareVersion> = await axios.get(
          `${apiBaseUrl}/software-version/${id}/`,
          {
            headers: this.getAuthHeaders(),
          }
        );
    ...
    async createSoftwareVersion(data: SoftwareVersionPayload) {
      ...
      try {
        ...
        const apiBaseUrl = useApiBaseUrl();
        const res: AxiosResponse<SoftwareVersion> = await axios.post(
          `${apiBaseUrl}/software-version/`,
          payload,
          {
            headers: this.getAuthHeaders(),
          }
    ...
    async updateSoftwareVersion(id: number, data: SoftwareVersionPayload) {
      ...
      try {
        ...
        const apiBaseUrl = useApiBaseUrl();
        const res: AxiosResponse<SoftwareVersion> = await axios.put(
          `${apiBaseUrl}/software-version/${id}/`,
          payload,
          {
            headers: this.getAuthHeaders(),
